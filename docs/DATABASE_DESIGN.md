...
This document outlines the design principles, architectural decisions, and rationale behind the Lumanitech ERP Finance database schema.
...
6. **API-First**: Designed to be accessed primarily through the Finance API

## Core Principles

### 1. Database Ownership

**The Finance API owns this database.**

- All schema changes must go through the Finance API team
- Other services access project data only via the Finance API
...
This database design balances simplicity, performance, and scalability while maintaining data integrity and supporting the needs of the Finance API. The forward-only migration strategy ensures safe schema evolution, and the clear separation of concerns allows for independent service scaling.
...
**Schema Version**: V003  
**Maintained By**: Finance API Team
