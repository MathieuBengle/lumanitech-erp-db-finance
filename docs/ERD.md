...
This document describes the relationships between tables in the Lumanitech ERP Finance database.
...
**Note**: These are logical foreign keys, not enforced by database constraints. The Finance API is responsible for validating user IDs before writing to the database.
...
- The Finance API should be notified (via event or API call)
